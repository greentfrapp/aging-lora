...
### 1.4.1 Leakage-audit methodology (contribution from this study)

A rigorous evaluation of single-cell foundation models against task-specific clocks requires cross-referencing each FM's pretraining manifest against every training and holdout cohort, and then footnoting each per-(model, cohort) result row with its leakage status. This project's leakage audit (`data/leakage_audit.csv`, `methods/leakage_audit_notes.md`, 2026-04-24) found that **direct-accession searches systematically miss overlaps when datasets have multiple deposit mirrors.** Specifically: scFoundation was confirmed to have ingested Stephenson et al. 2021 *Nature Medicine* (E-MTAB-10026) via the HCA-native Project ID `HCA-Covid19PBMC` (Hao et al. 2024 *Nat Methods* Supp Table 5 row 81); a direct search against the ArrayExpress accession returned zero hits. scRNA-seq datasets are routinely mirrored across CellxGene, HCA, ArrayExpress, GEO, SRA, and institutional portals — leakage audits in this space must check every mirror, not just the first-discovered accession. This is reported as a small methodological contribution in the Phase 3 preprint and central to the Phase 4 strict-clean vs. inclusive meta-analysis policy.
