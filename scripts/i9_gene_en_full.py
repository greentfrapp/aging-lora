"""I.9 — gene-EN 3-seed full curve for NK and B cell types.

Same pipeline as i7/i8 gene-EN scripts but parameterized over cell type
and covering caps {1, 5, 10, 20, 50, 100, 500, 1000} × 3 seeds × 2 folds.

Output: results/phase3/i9_gene_en_full.csv
"""
...
CELL_TYPE_TO_FILE = {"NK": "NK.h5ad", "B": "B.h5ad"}
...
TOP_N_HVG = 5000
CAPS = [1, 5, 10, 20, 50, 100, 500, 1000]
SEEDS = [0, 1, 2]
CELL_TYPES = ["NK", "B"]
OUT_CSV = Path("results/phase3/i9_gene_en_full.csv")
...
def _safe_pearson(pred, y):
    if pred.std() > 1e-3 and y.std() > 0:
        r, _ = pearsonr(pred, y)
...
def main():
    ...
    for cell_type in CELL_TYPES:
        for seed in SEEDS:
            for cap in CAPS:
                print(f"\n=== {cell_type} seed={seed} CAP={cap} ===", flush=True)
                for fold_id in ["loco_onek1k", "loco_terekhova"]:
                    ...
                    r = _safe_pearson(pred, eval_y)
                    mae = float(np.median(np.abs(pred - eval_y)))
                    ci = _bootstrap_ci(pred, eval_y, seed)
                    elapsed = time.time() - t0

                    rows.append({
                        "cell_type": cell_type, "seed": seed, "cap": cap, "fold": fold_id, "eval_cohort": eval_cohort,
                        "n_train": int(len(train_y)), "n_eval": int(len(eval_y)),
                        "alpha": alpha, "l1_ratio": l1, "n_nonzero_genes": n_nz,
                        "R": r, "MAE": mae, "R_ci_lo": ci[0], "R_ci_hi": ci[1],
                    })
                    print(f"  {cell_type} seed={seed} cap={cap:5d} {fold_id:14s} → {eval_cohort:12s}: R={r:+.3f} MAE={mae:.2f} ({elapsed:.0f}s)", flush=True)
                    ...
                    ar = _safe_pearson(apred, aida_y)
                    amae = float(np.median(np.abs(apred - aida_y)))
                    aci = _bootstrap_ci(apred, aida_y, seed)
                    rows.append({
                        "cell_type": cell_type, "seed": seed, "cap": cap, "fold": fold_id, "eval_cohort": "aida",
                        "n_train": int(len(train_y)), "n_eval": int(len(aida_y)),
                        "alpha": alpha, "l1_ratio": l1, "n_nonzero_genes": n_nz,
                        "R": ar, "MAE": amae, "R_ci_lo": aci[0], "R_ci_hi": aci[1],
                    })
                    print(f"  {cell_type} seed={seed} cap={cap:5d} {fold_id:14s} → AIDA       : R={ar:+.3f} MAE={amae:.2f}", flush=True)
    ...
    df = pd.DataFrame(rows)
    print(f"\n[I.9 gene-EN] wrote {len(df)} rows to {OUT_CSV}")

    print("\n=== I.9 gene-EN 3-seed mean AIDA R per cell_type × cap × fold ===")
    aida = df[df["eval_cohort"] == "aida"]
    for cell_type in CELL_TYPES:
        for fold in ["loco_onek1k", "loco_terekhova"]:
            sub = aida[(aida["cell_type"] == cell_type) & (aida["fold"] == fold)]
            if len(sub) == 0:
                continue
            print(f"\n{cell_type} fold={fold}:")
