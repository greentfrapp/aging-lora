"""I.8 — gene-EN 3-seed at cap=10 and cap=20 (manuscript-grade fillers).

Mirrors `i7_gene_en_low_cap.py` with CAPS = [10, 20] and includes the
robust NaN guard from the I.7 NaN fix (pred.std() > 1e-3 +
np.isfinite(r) post-check) — at small training sizes ElasticNetCV may
collapse to all-zero coefs, producing constant predictions and NaN
correlation. Treat as R=0 (model failed to learn).

Output: results/phase3/i8_gene_en_fillers.csv
"""
...
CAPS = [10, 20]
SEEDS = [0, 1, 2]
OUT_CSV = Path("results/phase3/i8_gene_en_fillers.csv")
...
def main():
    ...
    df = pd.DataFrame(rows)
    print(f"\n[I.8 gene-EN] wrote {len(df)} rows to {OUT_CSV}")

    print("\n=== I.8 gene-EN 3-seed mean AIDA R per cap × fold ===")
