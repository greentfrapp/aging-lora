"""I.7 — gene-EN 3-seed at cap=1 and cap=5.

Closes out the low-cap end of the matched-cap trajectory. Same pipeline
as i6_gene_en_3seed.py but with CAPS = [1, 5].

Output: results/phase3/i7_gene_en_low_cap.csv
"""
...
CAPS = [1, 5]
SEEDS = [0, 1, 2]
OUT_CSV = Path("results/phase3/i7_gene_en_low_cap.csv")
...
def main():
    ...
    df = pd.DataFrame(rows)
    print(f"\n[I.7 gene-EN] wrote {len(df)} rows to {OUT_CSV}")

    print("\n=== I.7 gene-EN 3-seed mean AIDA R per cap × fold ===")
