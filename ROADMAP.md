...
- [x] **Phase 2 — Baseline establishment.** See [phase-2](./roadmap/phase-2.md). *(Closed 2026-04-25. Panel revised mid-phase: sc-ImmuAging ships pretrained LASSO only — RF and PointNet weights are not in the public package; see `methods/loco_baselines.md`.)*
      Three pretrained external clocks scored on each LOCO holdout cohort. 45-row `results/baselines/loco_baseline_table.csv` = 3 baselines × 3 cohorts × 5 cell types, each row tagged with `leakage_status`, `chemistry_match_to_baseline_training`, `detectability_flag`. **Per-cell minimum-MAE baseline on the strict-clean Terekhova fold**: Pasta CD4T 8.0y, Pasta CD8T 7.6y, LASSO MONO 12.8y, LASSO NK 12.5y, Pasta B 10.9y. **Pasta CD4T R=0.78 / MAE=8.0y beats LASSO on MAE; Pasta B R=0.28 vs LASSO R=0.08 = chemistry-rescue baseline that Phase-4 FMs must beat.** scAgeClock surprisingly weak: R values below LASSO even on its own training cohorts (OneK1K, Stephenson) — a generalist deep clock does not automatically dominate a PBMC-specialist on a single tissue. RF/PointNet retraining and Pasta retraining both deferred to supplement (RF/PointNet need original 5-cohort training data, two of which are EGA-controlled; Pasta training code is not in the public repo).
...
4. **Phase 3 is the go/no-go gate for the primary claim.** *(Updated 2026-04-25 to reflect the Phase-2 panel revision.)* If the FM LoRA fine-tunes fail to improve over the **per-cell minimum-MAE baseline** from {LASSO, scAgeClock, Pasta-REG} by more than 5% relative m.a.e. on the CD4+ T LOCO primary folds, the project pivots before Phase 4 begins: (a) Phase 4 is narrowed to the zero-shot transfer and few-shot curve experiments only (dropping the full 4-model × 5-cell-type sweep to the best single model); (b) the paper is repositioned as an evaluation study — "Do scRNA-seq foundation models improve immune aging clocks beyond a published deep-learning clock (scAgeClock)?" — with the null fine-tuning result as the headline. The "FMs beat the *strongest* published baseline" framing makes a null result more publishable than the original "FMs beat LASSO" framing. The preprint posted in Phase 3 already frames this as a possibility, so the pivot requires only a revision rather than a retraction.
