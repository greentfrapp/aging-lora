...
- [ ] **Phase 2 — Baseline establishment.** See [phase-2](./roadmap/phase-2.md). *(Panel revised 2026-04-25 after upstream-availability check: sc-ImmuAging ships pretrained LASSO only — RF and PointNet weights are not in the public package; see `methods/loco_baselines.md`.)*
      Success: pretrained external clocks scored on each LOCO holdout. Panel = **{LASSO (sc-ImmuAging), scAgeClock (Xie 2026), Pasta-REG (Salignon 2025)}**. LASSO done 2026-04-24 — 15-row table at `results/baselines/loco_baseline_table.csv` with three stratification columns. scAgeClock pending: needs a 4-row leakage-audit follow-up (model × {OneK1K, Stephenson, Terekhova, AIDA}) appended to `data/leakage_audit.csv` before scoring is reported. Pasta-REG pending: R + `jsalignon/pasta` install; if R bridge is multi-day, defer Pasta to supplement. RF/PointNet retraining and Pasta retraining both deferred (sc-ImmuAging RF/PointNet need original 5-cohort training data; Pasta training code is not in the public repo). Phase 4 reports FM wins vs. the **per-cell minimum-MAE baseline** from this panel, not just LASSO.
