...
- [ ] **Phase 3 — CD4+ T pilot fine-tune & preprint.** See [phase-3](./roadmap/phase-3.md).
      ...
      *Phase-3-A B-cell and NK-cell extensions added 2026-04-28* after CD4+T close-out returned 0/3 wins — required to bound the negative claim ("0/N across cells with strong AND weak baselines") and to test the chemistry-rescue hypothesis that the kickoff §3 framed around B-cell / NK-cell. See [phase-3](./roadmap/phase-3.md) "Phase-3-A B-cell extension" and "Phase-3-A NK-cell extension".
      *Phase-3-A protocol-and-FM-class diagnostic added 2026-04-28* after B + NK partial results showed cell-type-specific anti-correlation — the per-cell fine-tune protocol confounds 4 variables (model class, feature space, unit of analysis, training objective). NK × loco_terekhova cancelled in favor of Variant 1 ridge probe + conditional Variant 2/3 + scFoundation FM-class diagnostic. See [phase-3](./roadmap/phase-3.md) "Phase-3-A protocol-and-FM-class diagnostic".
...
4. **Phase 3 is the go/no-go gate for the primary claim — now a TRI-headline gate.** *(Updated 2026-04-25 after the full Phase-2 add-on suite.)* The Phase-3 CD4+T pilot must clear the per-cell minimum-MAE baseline from a **4-comparator panel** {LASSO-pretrained, LASSO-retrained-3cohort, scAgeClock, Pasta-REG} by ≥10% relative m.a.e. on **three headline cells**: OneK1K CD4+T (target ≤8.5y, LASSO floor 9.4y), Terekhova CD4+T (target ≤7.2y, Pasta floor 8.0y — chemistry-rescue), and AIDA CD4+T (target ≤5.7y, Pasta floor 6.3y — cross-ancestry; tightest cell in entire baseline matrix). Including LASSO-retrained-3cohort directly addresses the "more training data" reviewer concern: both retrained-LASSO and FMs see the same 3-cohort corpus. AIDA scoring is one extra inference call per fine-tuned checkpoint (no additional training cost) and brings cross-ancestry results into the preprint before Phase 4 begins. Pre-Phase-2 success probability for headline win was ~80% against LASSO alone; revised post-Phase-2 estimates are **~70–80% on OneK1K, ~50–60% on Terekhova, ~30–40% on AIDA** (Pasta-REG is rank-norm-invariant + ancestry-invariant in a way that's hard to beat). Outcome aggregation: 3/3 wins → primary headline; 2/3 → strong primary; 1/3 → degraded claim ("FMs match-or-beat on [winning cell]; further investigation for [losing dimensions]"); 0/3 → pivot to evaluation-study framing. The preprint posted in Phase 3 already frames the match/loss possibilities, so a pivot requires revision not retraction.
