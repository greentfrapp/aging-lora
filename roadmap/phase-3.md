...
### Preprint headline gate (recorded 2026-04-25)

The preprint headline claim is "FMs beat the published immune-aging baseline on CD4+ T LOCO." With the Phase-2 panel revision (LASSO + scAgeClock + Pasta-REG), **the gate is "FMs beat the better of {LASSO, scAgeClock} per (cohort, cell type) cell"**, not just LASSO. If FMs beat LASSO but lose to scAgeClock on a cell, that result becomes "FMs match-but-don't-exceed scAgeClock" and the headline weakens to a tie. The gate keeps the paper from over-claiming when scAgeClock (a published 2026 deep-learning clock) already eats most of the gap.

A degraded-claim fallback is acceptable: if FMs lose to scAgeClock on `loco_terekhova` × CD4+ T, the preprint's headline can still report (a) the chemistry-robustness subfigure independently and (b) the strict-clean-AND-chemistry-match comparison on `loco_onek1k`, which is a cleaner FM-vs-LASSO contrast since scAgeClock × OneK1K is leakage-`overlapping` per `data/leakage_audit.csv` (CELLxGENE Census).
...
- **Beat-best-baseline gate evaluated:** for each (cohort, cell type) cell, compute relative MAE reduction of each FM vs. the *minimum* of {LASSO MAE, scAgeClock MAE, Pasta-REG MAE} on that cell. Headline claim "FMs beat published baselines by ≥10% relative MAE on CD4+ T loco_terekhova" requires the relative reduction to be ≥10% against the *better* baseline, not just against LASSO. If FMs only beat LASSO but lose to scAgeClock, the preprint reports it as "FMs match scAgeClock at lower training cost" rather than as a primary win.
...
- bioRxiv preprint posted within 10 weeks of project start; manuscript's headline figure is **the CD4+ T loco_terekhova result** for Geneformer + scFoundation + scGPT vs. **the better of {LASSO, scAgeClock} on a per-(cohort, cell type) basis**, with the full leakage-audit table (now including scAgeClock × 4 cohorts), the frozen split design, and the chemistry-robustness subfigure. At this point the paper is "minimal viable" regardless of whether criterion (a) is met.
