...
- [x] **Task I.7 (DONE 2026-05-01, see §49; extends I.6 to extreme low-cap): cap=1 and cap=5 × 3 seeds × {FM, gene-EN}.** Decision rule TRIGGERED: cap=5 FM-vs-gene-EN gap = +0.168 (loco_onek1k) / +0.187 (loco_terekhova), well above the +0.10 R "FM-as-low-cap-rescuer" threshold. **The matched-cap FM advantage peaks at cap=5, not cap=100.** Trajectory of FM-minus-gene-EN AIDA gap (loco_onek1k): cap=1 +0.124 → cap=5 +0.168 (peak) → cap=50 +0.081 → cap=100 +0.017 → cap=500 −0.002 → cap=1000 −0.005. cap=1 gene-EN is statistically zero (-0.001 / +0.034); FM at cap=1 still gets +0.12-0.15 R with high SD (0.15-0.20). Methodology reframe: "FM extracts ~0.17 R more age signal than bulk **when only 5 cells per donor are available**" — useful for rare cell types, low-throughput cohorts, and few-shot scenarios. By cap=500, FM and bulk are tied. ~30 min wall (parallel), ~$0.30 GPU. Closes out the low-cap end of the matched-cap trajectory. I.6 found the FM advantage at cap=50 (+0.08 R AIDA) and cap=100 (+0.02 R), tied at cap=500/1000. Extending downward tests where the FM per-cell-efficiency advantage maxes out — and whether cap=1 is degenerate (1 cell per donor → noisy embeddings).
  - **Implementation**:
