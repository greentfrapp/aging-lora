...
- [x] Leakage-audit table produced 2026-04-24: `data/leakage_audit.csv` + `methods/leakage_audit_notes.md`. 16 rows (4 models × 4 cohorts). **Key finding: Terekhova is `clean` for all four models (scGPT, Geneformer, scFoundation, UCE), making `loco_terekhova` the project's gold-standard LOCO fold. OneK1K and Stephenson are `overlapping` for scGPT and UCE (both pretrain on CellxGene Census, which includes both cohorts) and `unknown` for Geneformer and scFoundation (requires human supp-table lookup — see HUMAN_TASKS.md #4).** AIDA is clean across all models (2025 publication postdates every checkpoint).
- LOCO fold matrix frozen in `data/loco_folds.json`: three leave-one-cohort-out folds, each annotated with donor count. Folds with fewer than 80 held-out donors flagged as exploratory-only and excluded from the primary result matrix. **Primary LOCO folds (≥80 donors held out): OneK1K (981), Terekhova (166).** Exploratory-only: Stephenson (29). For the two primary folds, report naive (chemistry-uncorrected) and chemistry-corrected MAE side-by-side since OneK1K+Stephenson are 10x 3' while Terekhova is 10x 5'. **Pair each result row with the `leakage_status` from `data/leakage_audit.csv`.** For `loco_onek1k`, scGPT and UCE rows must carry an asterisk-footnote noting training-set overlap; this makes `loco_terekhova` the headline generalization result of the paper.
...
- [ ] Task: Run pretraining-corpus leakage audit. Extract each model's published pretraining dataset manifest (at GEO/CELLxGENE collection-ID granularity). Cross-reference against the three training cohorts (CellxGene `3faad104` OneK1K — flag GEO `GSE196830` as the same underlying dataset; CellxGene `ddfad306` Stephenson; Synapse `syn49637038` Terekhova) and against AIDA (CELLxGENE `ced320a1-29f3-47c1-a735-513c7084d508`). Write the result to `data/leakage_audit.csv` with columns: model, cohort_id, cohort_name, status ∈ {clean, overlapping, unknown}, evidence_url. Identify any (model, cohort) pairs that are overlapping or unknown; flag the corresponding LOCO folds for leakage-restricted reporting. Done when the CSV is complete and committed.
