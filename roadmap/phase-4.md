...
- LoRA fine-tune result matrix complete for all four models × five cell types × surviving primary LOCO folds (≥3 seeds per cell); per-fold per-cell-type m.a.e. recorded in `results/phase4/full_loco_matrix.csv` with **three stratification columns** (`leakage_status`, `chemistry_match_to_baseline_training`, `detectability_flag`) on every row; forest plot produced showing per-fold Fisher-z Pearson R effect sizes, with clean and overlapping rows rendered in distinct glyphs and shifted-vs-match chemistry rendered as complementary markers.
- Dual-regime few-shot downsampling curve produced for CD4+ T (chemistry-robust pre-trained baseline — R=0.82 on 5' Terekhova) and B cells (chemistry-collapsed pre-trained baseline — R=0.08 on 5' Terekhova). On each cell type, run downsampling to 75%, 50%, 25%, 10% of training donors for both LASSO and the best-performing LoRA foundation model. The B-cell curve directly tests the Phase-1 chemistry-rescue hypothesis; the CD4+ T curve tests the original low-data claim under a matched-chemistry baseline. Crossover thresholds (if any) documented per cell type.
...
- [ ] Task: Run dual-regime few-shot downsampling curve (CD4+ T and B cells). For each cell type, downsample training donors to 75%, 50%, 25%, 10% of the full training set, and fit both LASSO and the best-performing LoRA foundation model at each level on each primary LOCO fold. Record per-fold m.a.e. For B cells specifically, include a `loco_terekhova` (5' chemistry) evaluation arm alongside the `loco_onek1k` arm — this is the chemistry-rescue test: if Geneformer's 5' B-cell R exceeds the pre-trained LASSO's R=0.08 at every sample-size level, the chemistry-inclusive meta-analysis carries a separate headline claim. Plot m.a.e. vs. number of labelled donors for both cell types and both model classes, identifying any crossover points. Done when `results/phase4/fewshot_curves/{cd4t,bcell}_{onek1k,terekhova}.csv` are committed and crossover thresholds are documented in `methods/fewshot_analysis.md`.
