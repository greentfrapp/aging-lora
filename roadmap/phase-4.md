...
Phase 4 reports the FM full-LOCO matrix against the **Phase-2 baseline panel of 4 comparators**: LASSO-pretrained-5cohort, LASSO-retrained-3cohort, scAgeClock, Pasta-REG. Each (cohort × cell type) cell carries four baseline numbers; the FM's "headline win" is computed against the *per-cell minimum-MAE baseline* (the "beat-best-baseline" min-of-4 rule from Phase 3). The LASSO-retrained-3cohort comparator is the methodologically-symmetric apples-to-apples baseline (same training corpus as the FMs) and addresses the "FM win is just from more training data" reviewer concern. The `leakage_status` stratification column extends to scAgeClock rows: scAgeClock × {OneK1K, Stephenson} are `overlapping` (CELLxGENE Census), scAgeClock × {Terekhova, AIDA} are `clean` (recorded in `data/leakage_audit.csv` from the Phase 2 Task 2.2 audit). For `loco_terekhova` AND for AIDA evaluation, all four baselines and all four FMs are leakage-clean → these are the strict-clean folds where the meta-analysis includes scAgeClock as a comparator.
