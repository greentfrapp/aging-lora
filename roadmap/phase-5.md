...
- Benchmark harness v1 released alongside the revised preprint: `pytest`-runnable suite covering frozen LOCO splits, checkpoint hashes, scanpy/PyTorch/peft version pins, and one end-to-end training + evaluation run per cell type on a subset of donors. GitHub release tag `v1.0.0` with DOI (Zenodo or equivalent).
